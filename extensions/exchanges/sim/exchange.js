...
module.exports = function sim (conf, s) {
  ...
  try {
    ...
    let fallbackProduct = {
      asset: so.selector.asset,
      currency: so.selector.currency,
      product_id: so.selector.product_id,
      normalized: so.selector.normalized,
      increment: so.increment || '0.00000001',
      asset_increment: so.asset_increment || '0.00000001',
      label: so.selector.product_id
    }
