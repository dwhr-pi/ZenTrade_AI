...
module.exports = function (program, conf) {
  program
    ...
    .action(function (selector, cmd) {
      ...
      var command_args = [path.resolve(__dirname, '..', 'zenbot.js'), 'backfill', so.selector.normalized, '--days', days || 1]
      if (cmd.conf) {
        command_args.push('--conf', cmd.conf)
      }
      var backfiller = spawn(process.execPath, command_args, { cwd: path.resolve(__dirname, '..') })
