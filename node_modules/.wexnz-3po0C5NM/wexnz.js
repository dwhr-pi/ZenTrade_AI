...
function WEXNZ(key, secret) {
  this.key = key
  this.secret = secret
  this.urlPost = 'https://wex.nz:443/tapi'
  this.urlGet = 'https://wex.nz:443/api/2/'
  this.nonce = WEXNZ.getTimestamp(Date.now())
}
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.activeOrders = function(params, callback) {
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.trade = function(params, callback) {
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.cancelOrder = function(orderId, callback) {
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.query = function(method, params, callback) {
  var content = {
    'method': method,
    'nonce': ++this.nonce,
  }
  ...
  if (!!params && typeof(params) == 'object') {
    Object.keys(params).forEach(function (key) {
      if (key == 'since' || key == 'end') {
        value = WEXNZ.getTimestamp(params[key])
      }
      else {
        value = params[key]
      }
  ...
  var req = https.request(options, function(res) {
    ...
    res.on('end', function() {
      WEXNZ.responseHandler(null, data, callback)
    })
  })

  req.on('error', function(err) {
    WEXNZ.responseHandler(err, null, callback)
  })
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.getHTTPS = function(getUrl, callback) {
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.trades = function(params, callback) {
  if (!params) params = {}
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.depth = function(params, callback) {
  if (!params) params = {}
  if (!params.count) params.count = 100
  if (!params.pair) params.pair = 'btc_usd'
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.ticker = function(params, callback) {
  if (!params) params = {}
  if (!params.pair) params.pair = 'btc_usd'
 ...
 * @param {Function} callback(err, data)
 */
WEXNZ.prototype.fee = function(params, callback) {
  if (!params) params = {}
  if (!params.pair) params.pair = 'btc_usd'
...
/**
 * Helper function to handle WEXNZ HTTP responses and errors
 */
WEXNZ.responseHandler = function(err, data, callback) {
  if (err) {
    callback(err, null)
  } else {
    var result = null
 ...
 * @param {Mixed} time
 */
WEXNZ.getTimestamp = function(time) {
  if (util.isDate(time)) {
    return Math.round(time.getTime() / 1000)
  }
  if (typeof time == 'string') {
    return WEXNZ.getTimestamp(new Date(time))
  }
