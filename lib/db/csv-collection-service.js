      ...
      bulkUpsert: function (docs, cb) {
        var promise = Promise.resolve().then(function () {
          ;(docs || []).forEach(function (doc) {
            ...
            var index = csvManager.cache[collectionName].findIndex(function (item) {
            ...
            if (index >= 0) {
              var existing = csvManager.cache[collectionName][index]
              if (typeof existing._natural === 'number' && typeof nextDoc._natural !== 'number') {
                nextDoc._natural = existing._natural
              }
              csvManager.cache[collectionName][index] = nextDoc
            } else {
              if (typeof nextDoc._natural !== 'number') {
                nextDoc._natural = nextNatural(collectionName)
              }
              csvManager.cache[collectionName].push(nextDoc)
            }
          })

          return csvManager.syncCollection(collectionName).then(function () {
            return {
              insertedCount: (docs || []).length,
