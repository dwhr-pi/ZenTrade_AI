...
module.exports = function (s, conf) {
  ...
  function initBuffer (trade) {
    ...
    if ((so.mode === 'sim' || so.mode === 'paper') && !s.start_capital) {
      initializeStartingCapital(trade.price)
    }
  }

  function initializeStartingCapital (referencePrice) {
    let price = Number(referencePrice)
    if (!isFinite(price) || price <= 0) return

    s.asset_capital = n(s.balance.asset || 0).multiply(price).value()
    let deposit = so.deposit ? Math.max(0, n(so.deposit).subtract(s.asset_capital).value()) : n(s.balance.currency || 0).value()
    s.balance.deposit = n(deposit < (s.balance.currency || 0) ? deposit : (s.balance.currency || 0)).value()

    if (!s.start_capital) {
      s.start_price = price
      s.start_capital = n(s.balance.deposit).add(s.asset_capital).value()
      s.real_capital = n(s.balance.currency || 0).add(s.asset_capital).value()
      s.net_currency = n(s.balance.deposit).value()
    }
  }

  function nextBuyForQuote(s, quote) {
    let price_format = getFormatForIncrement(s.product.increment, '0.00')
    if (s.next_buy_price)
      return n(s.next_buy_price).format(price_format, Math.floor)
    else
      return n(quote.bid).subtract(n(quote.bid).multiply(s.options.markdown_buy_pct / 100)).format(price_format, Math.floor)
  }

  function nextSellForQuote(s, quote) {
    let price_format = getFormatForIncrement(s.product.increment, '0.00')
    if (s.next_sell_price)
      return n(s.next_sell_price).format(price_format, Math.ceil)
    else
      return n(quote.ask).add(n(quote.ask).multiply(s.options.markup_sell_pct / 100)).format(price_format, Math.ceil)
  }

  function getFormatForIncrement(increment, fallback) {
    if (!increment) return fallback
    let increment_str = String(increment)
    let dot_index = increment_str.indexOf('.')
    if (dot_index === -1) return '0'
    let decimals = increment_str.length - dot_index - 1
  ...
  function syncBalance (cb) {
    ...
    s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
      ...
      getQuote(function (err, quote) {
        ...
        let wasUninitialized = !s.start_capital
        initializeStartingCapital(quote.ask)
        if (wasUninitialized && s.start_capital) {
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker, reverseCalled) {
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (signal === 'buy') {
        ...
        if (buy_pct + fee < 100) {
          size = n(tradeable_balance).divide(price).format(getFormatForIncrement(s.product.asset_increment, '0.00000000'))
        } else {
          size = n(trade_balance).subtract(expected_fee).divide(price).format(getFormatForIncrement(s.product.asset_increment, '0.00000000'))
        }
        ...
        size = n(s.balance.asset).multiply(sell_pct / 100).format(getFormatForIncrement(s.product.asset_increment, '0.00000000'))
        ...
        s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
          if (api_order) {
            ...
            if (api_order.filled_size) {
              order.remaining_size = n(order.size).subtract(api_order.filled_size).format(getFormatForIncrement(s.product.asset_increment, '0.00000000'))
            }
          }
